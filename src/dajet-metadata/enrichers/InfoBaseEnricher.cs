    ...
    public sealed class InfoBaseEnricher : IContentEnricher
    {
        ...
        private void ConfigureCommonObjects(InfoBase infoBase, ConfigObject config)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    ConfigObject
                        collection = config.GetObject(new[] { 3, 1, i + offset }); // узел объекта метаданных

                    Guid uuid = collection.GetUuid(new[] { 0 }); // идентификатор типа объекта метаданных

                    if (uuid == new Guid(
                            "15794563-ccec-41f6-a83c-ec5f7b9a5bc1")) // идентификатор коллекции общих реквизитов
                    {
                        ConfigureSharedProperties(infoBase, collection); // Общие реквизиты
                    }
                    else if
                        (uuid == new Guid(
                            "c045099e-13b9-4fb6-9d50-fca00202971e")) // идентификатор коллекции определяемых типов
                    {
        ...
        private void ConfigureSharedProperty(SharedProperty property, InfoBase infoBase)
        {
            ...
            while (uuidIndex <= count)
            {
                ...
                SharedPropertyUsage usage =
                    (SharedPropertyUsage)cfo.GetInt32(new[] { 1, 2, uuidIndex + usageOffset, 1 });
